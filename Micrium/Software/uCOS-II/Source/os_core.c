    ...
    if (OSIntNesting == 0u) {                          /* Schedule only if all ISRs done and ...       */
        if (OSLockNesting == 0u) {                     /* ... scheduler is not locked                  */
            ...
            if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy     */
                ...
                //printf("TICK %d OSPrioHighRdy: %d OSPrioCur %d\n",OSTime, TaskParameter[OSPrioHighRdy-1].TaskID, TaskParameter[OSPrioCur - 1].TaskID);
#if OS_TASK_CREATE_EXT_EN > 0u
#if defined(OS_TLS_TBL_SIZE) && (OS_TLS_TBL_SIZE > 0u)
                OS_TLS_TaskSw();
#endif
                   ...
                   INT16U   opt)
{
    ...
    if (ptcb != (OS_TCB *)0) {
        ...
        //PA1-1
         //if ptcb==IdleTask print ptcb->priority
        if (ptcb->OSTCBPrio == OS_TASK_IDLE_PRIO) {
            printf("Task[%d] created, TCB Address %x\n", ptcb->OSTCBPrio, ptcb);
